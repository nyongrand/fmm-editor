...
{
    /// <summary>
    /// Parses and manages coach / scout / physios data from binary files.
    /// </summary>
    public class CoachParser : IParser<Coach>
    {
        private readonly List<Coach> items;
        ...
        /// List of all items
        /// </summary>
        public IReadOnlyList<Coach> Items => items.AsReadOnly();

        /// <summary>
        /// Initializes a new instance of the <see cref="CoachParser"/> class.
        /// </summary>
        ...
        private CoachParser(string path, BinaryReaderEx reader)
        {
            FilePath = path;
            Header = reader.ReadBytes(8);
            Count = reader.ReadInt32();
            items = new List<Coach>(Count);
        }
        ...
        /// <returns>A task that represents the asynchronous operation. The task result contains the loaded <see cref="CoachParser"/> instance.</returns>
        public static async Task<CoachParser> Load(string path)
        {
            using var fs = File.OpenRead(path);
            using var reader = new BinaryReaderEx(fs);
            var parser = new CoachParser(path, reader);

            while (fs.Position < fs.Length)
            {
                var item = new Coach(reader);
