...
namespace FMMLibrary
{
    ...
    /// </summary>
    public class OfficialParser : IParser<Official>
    {
        private readonly List<Official> items;
        ...
        /// List of all items
        /// </summary>
        public IReadOnlyList<Official> Items => items.AsReadOnly();

        /// <summary>
        /// Initializes a new instance of the <see cref="OfficialParser"/> class.
        /// </summary>
        ...
        private OfficialParser(string path, BinaryReaderEx reader)
        {
            FilePath = path;
            Header = reader.ReadBytes(8);
            Count = reader.ReadInt32();
            items = new List<Official>(Count);
        }
        ...
        /// <returns>A task that represents the asynchronous operation. The task result contains the loaded <see cref="OfficialParser"/> instance.</returns>
        public static async Task<OfficialParser> Load(string path)
        {
            using var fs = File.OpenRead(path);
            using var reader = new BinaryReaderEx(fs);
            var parser = new OfficialParser(path, reader);

            while (fs.Position < fs.Length)
            {
                var item = new Official(reader);
